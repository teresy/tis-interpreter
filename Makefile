...
INCLUDES=$(addprefix -I , $(FRAMAC_SRC_DIRS)) -I $(PLUGIN_LIB_DIR) -I lib -I devel_tools
...
PLUGIN_EXTRA_DIRS:=engine values domains domains/cvalue domains/apron \
	legacy slevel utils gui_files structure
PLUGIN_CMO:= slevel/split_strategy value_parameters \
	...
	domains/cvalue/locals_scoping utils/state_import \
	...
	alarmset eval \
	structure/simple_vector structure/vector_hetero \
	structure/key structure/structure \
	...
	engine/abstractions engine/compute_functions register \
	 $(VALUE_GUI_AUX)
PLUGIN_CMI:= domains/abstract_domain
...
PLUGIN_BFLAGS:= $(shell ocamlfind query -i-format -r apron.octMPQ apron.boxMPQ apron.polkaMPQ apron.apron gmp)
PLUGIN_OFLAGS:= $(shell ocamlfind query -i-format -r apron.octMPQ apron.boxMPQ apron.polkaMPQ apron.apron gmp)
# PLUGIN_EXTRA_BYTE := $(shell ocamlfind query -a-format apron.apron apron.octMPQ gmp -predicates byte)
...
ifeq ("$(CLIENT)","")
	...
	$(RM) ../$(DISTRIB).tar.gz
	$(PRINT) "Updating files to archive with $(DISTRIB_HEADERS) headers NOT"
