  ...
  module Table_By_Slevel =
    Cil_state_builder.Stmt_hashtbl
      (Datatype.Pair
         ...
         (Value_types.Callstack.Hashtbl.Make(Datatype.Int)))
      (struct
  ...
  *)
  module Called_Functions_By_Slevel =
    State_builder.Hashtbl(Kernel_function.Hashtbl)
      (Value_types.Callstack.Hashtbl.Make(Datatype.List(Cvalue.Model)))
      (struct
...
*)
  ...
  let update_slevel_table stmt cs (array, widen) =
    let module H = Value_types.Callstack.Hashtbl in
...
module From = struct
  ...
  module PathDeps = struct
    let iter = mk_fun "From.PathDeps.iter"
    let find = mk_fun "From.PathDeps.find"
  end

  module MemDeps = struct
    let iter = mk_fun "From.MemDeps.iter"
    let find = mk_fun "From.MemDeps.find"
  end
end
