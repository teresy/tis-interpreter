...
let warn warn_mode alarm status = match alarm with
  ...
  | Alarms.Division_overflow _ ->
    do_warn warn_mode.others alarm status "division overflow"
  ...
  | Alarms.Index_in_address _ ->
    do_warn warn_mode.others alarm status
      "Out of bounds index in address"

  | Alarms.Pointer_arithmetic _ ->
    do_warn warn_mode.others alarm status
      "Pointer arithmetic"
