...
= struct
  ...
  type location = Base.SetLattice.t
  ...
  let compute state expr typ =
    let top = `Value (Interval.top, ()), Alarmset.all in
  ...
  let kill_bases bases state = match bases with
  ...
  = struct
      ...
      let gather_constraints exp record acc =
        ...
        then
          try
            ...
            let cons = record.value.v >>- fun interval ->
    ...
    let default_call _stmt _call _state =
  ...
  let compute_using_specification _ (kf, _) state =
    ...
    else `Value [{ post_state = top; summary = (); returned_value = None }]
...
module Conversion = struct
  type extended_value = Main_values.Interval.t
  type extended_location = Precise_locs.precise_location
  ...
  let extend_val interval =
    try Some (convert interval)
    with Out_of_Scope -> None

  let integer_to_scalar positive = function
    | None -> Scalar.of_infty positive
    | Some integer -> Scalar.of_mpqf (Mpqf.of_string (Integer.to_string integer))

  let restrict_val = function
    | None -> Interval.top
    | Some ival ->
      try
        let min, max = Ival.min_and_max ival in
        let min = integer_to_scalar (-1) min and max = integer_to_scalar 1 max in
        Interval.of_scalar min max
      with
        Cvalue.V.Not_based_on_null -> Interval.top

  let restrict_loc precise_loc =
    let location = Precise_locs.imprecise_location precise_loc in
    Locations.(Location_Bits.get_bases location.loc)
...
module Octagon = Domain_lift.Make (Make (Apron_Octagon)) (Conversion)
module Box = Domain_lift.Make (Make (Apron_Box)) (Conversion)
module Polka_Loose = Domain_lift.Make (Make (Apron_Polka_Loose)) (Conversion)
module Polka_Strict = Domain_lift.Make (Make (Apron_Polka_Strict)) (Conversion)
module Polka_Equalities =
  Domain_lift.Make (Make (Apron_Polka_Equalities)) (Conversion)
