...
let reinterpret_va_list ~with_alarms v =
    ...
    V.fold_i (fun (base : Base.t) (offset : Ival.t) cvalue_acc ->
      ...
      begin
        ...
        Valarms.do_warn with_alarms.CilE.imprecision_tracing (fun () ->
          Value_util.warning_once_current
            "spotted a va_list value which has a non-zero offset %a, this is \
             unexpected as va_lists in arrays and structures are not \
             supported%t"
            Ival.pretty offset
      ...
      begin
        ...
        in
        (* If the type of the base is not va_list compatible emit a warning. *)
        Valarms.do_warn with_alarms.CilE.imprecision_tracing (fun () ->
          Value_util.warning_once_current
  ...
  with V.Error_Top ->
    Value_parameters.fatal ~current:true "a va_list with a top value \
      encountered!%t" Value_util.pp_callstack
...
let eval_binop_int ~with_alarms ~te1 ev1 op ev2 =
  let r = match op with
    ...
    | Eq | Ne | Ge | Le | Gt | Lt as op ->
      ...
      (* CHECK: Does "warn" actually mean that we have 2 (uncomparable) pointers?... I assume that. *)
        ...
        begin
          if not precise_and_valid then
            ...
            Value_parameters.warning ~current:true
              "Pointers %a and/or %a are %s and an ordering couldn't be determined."
              Cvalue.V.pretty ev1 Cvalue.V.pretty ev2 problem
        end;
        comp_result
      else
