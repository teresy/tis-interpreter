...
let do_store_computed_call (callsite: Value_types.call_site) input_state actuals callres () =
...
let store_computed_call callsite input_state actuals callres =
  Value_stat.measure_memexec (do_store_computed_call callsite input_state actuals callres)
...
let do_reuse_previous_call (kf, _ as _callsite: Value_types.call_site) state actuals () =
  try
