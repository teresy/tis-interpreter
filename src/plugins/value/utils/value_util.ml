...
let call_stack = ref Value_types.Shared_callstack.empty
(* let call_stack_for_callbacks : (kernel_function * kinstr) list ref = ref [] *)

let clear_call_stack () =
  call_stack := Value_types.Shared_callstack.empty

let pop_call_stack () =
  Value_perf.stop_doing (Value_types.Shared_callstack.get !call_stack);
  call_stack := Value_types.Shared_callstack.pop !call_stack
;;

let push_call_stack kf ki =
  call_stack := Value_types.Shared_callstack.push (kf,ki) !call_stack;
  Value_perf.start_doing (Value_types.Shared_callstack.get !call_stack)
;;


let current_kf () = 
  let (kf,_) = (List.hd (Value_types.Shared_callstack.get !call_stack)) in kf;;

let call_stack () = Value_types.Shared_callstack.get !call_stack
...
module DegenerationPoints =
  Cil_state_builder.Stmt_hashtbl
    (Datatype.Pair(Datatype.Bool)
       (Value_types.Callstack.Hashtbl.Make (Datatype.Bool)))
    (struct
...
module JoinPoints =
  Cil_state_builder.Stmt_hashtbl
    (Datatype.Pair
       (Value_types.Callstack.Hashtbl.Make
