...
let backward_add_ptr typ ~res_value ~v1 ~v2 pos =
  try
    let ptr_size = Bit_utils.osizeof_pointed typ in
    (* v1 == res -/+ v2 *)
    let factor_v1 = if pos then Int_Base.neg ptr_size else ptr_size in
    let v1' = V.add_untyped factor_v1 res_value v2 in
    (* v2' == +/- (res - v1) *)
    let d, _ = V.sub_untyped_pointwise res_value v1 in
    let scale = Int_Base.project ptr_size in
    let d = Ival.scale_div ~pos:false scale d in
    let v2' = V.inject_ival (if pos then d else Ival.neg_int d) in
    Some (v1', v2')
  with Int_Base.Error_Top -> None
...
let backward_cast ~src_typ ~dst_typ ~src_val ~dst_val =
  ...
  | TInt _, TFloat (fkind, _) ->
    ...
    Some (V.cast_float_to_int_inverse ~single_precision dst_val)

  | TFloat (fkind, _), TInt _ ->
    let single_precision = fkind = FFloat in
    cast_int_to_float_inverse ~single_precision dst_val
