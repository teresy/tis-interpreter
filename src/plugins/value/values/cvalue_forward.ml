      ...
      | _ ->
        ...
        then
          ...
          try
            ...
            V.narrow all_values interpreted_e, alarms
          with V.Not_based_on_null -> interpreted_e, alarms
        else begin
          if signed then
...
let division_alarms expr value =
  ...
  then
    ...
    try ignore (Cvalue.V.project_ival value); alarms
    with Cvalue.V.Not_based_on_null ->
      Alarmset.add (Alarms.Pointer_comparison (None, expr)) alarms
  else Alarmset.none
...
let forward_unop ~check_overflow ~context typ op value =
  ...
  | BNot ->
    let value =
      try
        let ival = V.project_ival value in
        V.inject_ival (Ival.apply_set_unary "~" Integer.lognot ival)
      with V.Not_based_on_null -> V.topify_arith_origin value
    in
    value, Alarmset.none
  | LNot ->
    let warn =
