...
class virtual visitor main =
  object(self)
    ...
    method vself = (** Print a cluster *)
      begin
        List.iter self#vcomp (List.rev main.c_records) ;
        List.iter self#vtype (List.rev main.c_types) ;
        List.iter (fun d -> self#vsymbol d.d_lfun) (List.rev main.c_symbols) ;
        List.iter (fun l -> self#vdlemma l) (List.rev main.c_lemmas) ;
      end
